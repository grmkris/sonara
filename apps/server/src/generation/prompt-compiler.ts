...
export function serializeResolvedScene(rs: ResolvedScene): string {
  ...
  for (let i = 1; i < rs.subjects.length; i++) {
    ...
    if (s.position?.trim()) parts.push(s.position.trim());
    if (s.action?.trim()) parts.push(s.action.trim());
    tail.push(parts.join(" "));
  }

  if (rs.composition.trim()) tail.push(rs.composition.trim());
  if (rs.style.trim()) tail.push(rs.style.trim());
  if (rs.background.trim()) tail.push(rs.background.trim());
  if (rs.lighting.trim()) tail.push(rs.lighting.trim());
  ...
  if (camParts.length > 0) tail.push(camParts.join(", "));

  if (rs.mood.trim()) tail.push(rs.mood.trim());
  ...
  if (rs.color_palette.length > 0) {
    ...
    if (paletteText.length > 0) tail.push(`palette: ${paletteText}`);
  }

  for (const mod of rs.drift_modifiers) {
    const m = mod.trim();
    if (m.length > 0) tail.push(m);
  }
